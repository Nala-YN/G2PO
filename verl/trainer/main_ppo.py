...
class TaskRunner:
    def run(self, config):
        ...
        if config.env.get("use_embedding", False):
            from verl.workers.embedding.embedding_worker import EmbeddingWorker

            role_worker_mapping[Role.Embedding] = ray.remote(EmbeddingWorker)
            mapping[Role.Embedding] = global_pool_id
