...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, step_advantage_w=1.0, mode="mean_std_norm", **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns = core_g2po.compute_g2po_outcome_advantage(
            ...
            step_rewards=data.non_tensor_batch['step_rewards'], # for step group reward computing
            response_mask=data.batch['response_mask'],
            index=data.non_tensor_batch['uid'],
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.use_embedding_worker = getattr(config.env, "use_embedding", False) and Role.Embedding in role_worker_mapping
    ...
    def init_workers(self):
        ...
        if self.use_embedding_worker:
            resource_pool = self.resource_pool_manager.get_resource_pool(Role.Embedding)
            embedding_cls = RayClassWithInitArgs(self.role_worker_mapping[Role.Embedding], config=self.config.env)
            self.resource_pool_to_cls[resource_pool]["embedding"] = embedding_cls
        ...
        if self.use_embedding_worker:
            self.embedding_wg = all_wg["embedding"]
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.G2PO:
                        step_rewards, group_idx, next_group_idx = core_g2po.compute_group_aggregation_values(batch=batch)
                        batch.non_tensor_batch['step_rewards'] = step_rewards
                        batch.non_tensor_batch['group_idx'] = group_idx
                        batch.non_tensor_batch['next_group_idx'] = next_group_idx
                    ...
                    with _timer("adv", timing_raw):
                        ...
                        batch = compute_advantage(
                            ...
                            step_advantage_w=self.config.algorithm.g2po.step_advantage_w,
                            mode=self.config.algorithm.g2po.mode
                        )
