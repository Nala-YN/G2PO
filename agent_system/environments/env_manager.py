...
class AlfWorldEnvironmentManager(EnvironmentManagerBase):
    ...
    def reset(self, kwargs):
        ...
        self.history_obs=[[full_text_obs[i]] for i in range(len(text_obs))]
    ...
    def step(self, text_actions: List[str]):
        ...
        for i in range(len(admissible_commands)):
            ...
            is_invisible_obs=len(self.history_obs[i])>=2 and np.array([self.history_obs[i][-2].startswith(pattern) for pattern in PATTERNS]).any()
            if is_invisible_obs:
                text_obs[i]=" ".join(self.history_obs[i][-2:])
            else:
                text_obs[i]=self.history_obs[i][-1]
...
class WebshopEnvironmentManager(EnvironmentManagerBase):
    ...
    def judge_if_in_detail(self, text_obs: List[str], infos: List[List[str]], init: bool = False) -> List[str]:
        judge_flag = []
        ...
        for i in range(len(text_obs)):
            ...
            if "buy now" in reformatted_available_actions.lower():
                judge_flag.append(True)
            else:
                judge_flag.append(False)
...
class AppWorldEnvironmentManager(EnvironmentManagerBase):
    ...
    def reset(self, kwargs):
        ...
        anchor_obs=["" for _ in range(len(text_obs))]
    ...
    def step(self, text_actions: List[str]):
        ...
        for i, info in enumerate(infos):
            info['is_action_valid'] = to_numpy(valids[i])
        #anchor_obs=[actions[i]+obs if "Execution successful" in obs else obs for i,obs in enumerate(text_obs)]
        anchor_obs=[]
        for i, obs in enumerate(text_obs):
            #new_obs=obs.split("Execution failed")[0].strip()
            new_obs=obs
            anchor_obs.append(new_obs)
    ...
    def build_text_obs(self, text_obs: List[str], init: bool = False) -> List[str]:
        ...
        if init and self.supervisors is not None:
            ...
            for i in range(len(text_obs)):
                ...
                if len(action_history) > 27000:
                    action_history = "... " + action_history[-27000:]
